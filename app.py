...
model = load_model('C:/Users/Asus/Desktop/Flask/Manhole_path/model.h5')

def allowed_file(filename):
    return '.' in filename and filename.rsplit('.', 1)[1].lower() in ALLOWED_EXTENSIONS

def predict_manhole(X):
    ...
    if np.array_equal(y_pred[0], [1, 0]):
        return "Manhole is predicted to be Closed"
    else:
        return "Manhole is predicted to be Open"
...
def displayImage():
    ...
    for img_file_path in img_file_paths:
        ...
        if X is None or X.size == 0:
            ...
            predictions[img_file_path] = predict_manhole(X)
